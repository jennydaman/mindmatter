...
export class BackgroundModule {
    ...
    constructor() {
        ...
        chrome.tabs.onRemoved.addListener(tabID => {
            if (tabID === this.questionSingleton)
                this.questionSingleton = null;
            // don't clear this.siteQueue = null
            // in case user does ctrl+shift+T?
        });
    ...
    attachRefreshHandler(refresher, interval = 6.048e8) {
        chrome.runtime.onStartup.addListener(() => {
            // TODO restore cooldown state on startup
            chrome.storage.local.remove('cooldown_lock');
            //refresh once a week
            this.refreshTimer = setInterval(refresher, interval);
        });
    }

    cooldownHandler(changes) {
        if (changes.cooldown_lock) {
            if (changes.cooldown_lock.newValue) {
                chrome.storage.sync.get('cooldown_info', items => {
                    ...
                    notif('Mind Matter',
                        `You are correct! I'll leave you alone for ${items.cooldown_info.english}.`);
                });
            }
